...
public class AccountService(UserManager<UserEntity> userManager, DataContext context, IConfiguration configuration)
{
    ...
    public async Task<bool> ChangePasswordAsync(ClaimsPrincipal userClaims, ChangePasswordForm model)
    {
        try
        {
    ...
    public async Task<bool> DeleteAsync(ClaimsPrincipal userClaims)
    {
        try
        {
            var nameIdentifier = userClaims.FindFirst(ClaimTypes.NameIdentifier)!.Value;
